...
Takto připravené struktura je již připraveno pro ukládání a načítání. Z tohoto důvodu bylo využito funkcí \uv{save} a \uv{load}. Obě tyto funkce jsou navíc podmíněny a to tak, že v případě funkce \uv{save} je do konce simulace ukládána i dočasná iterační proměnná. Jak je již patrné právě tato proměnná by měla být také obnovena při načítání dat. S takto připravenou části zbývá vytvořit jedinné a to komunikace s uživatelem. 
\chapter{Výsledky}
\label{ch:vysledky}
